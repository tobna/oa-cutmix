...
class AutoAugment(object):
    ...
    def __init__(self, policies: Union[str, List[List[dict]]], hparams: dict = _HPARAMS_DEFAULT):
        if isinstance(policies, str):
            assert (
                policies in AUTOAUG_POLICIES
            ), f"Invalid policies, please choose from {list(AUTOAUG_POLICIES.keys())}."
        ...
        for policy in policies:
            ...
            for augment in policy:
                assert (
                    isinstance(augment, dict) and "type" in augment
    ...
    def apply_pair(self, img, mask):
        ...
        if hasattr(sub_policy, "apply_pair"):
            return sub_policy.apply_pair(img, mask)

        # Fallback: apply sub_policy to image only (for color transforms)
        img = sub_policy(np.array(img))
...
class RandAugment(object):
    ...
    def __init__(
        ...
        num_policies: int,
        magnitude_level: int,
        magnitude_std: Union[Number, str] = 0.0,
        total_level: int = 30,
        ...
        if isinstance(policies, str):
            assert (
                policies in RANDAUG_POLICIES
            ), f"Invalid policies, please choose from {list(RANDAUG_POLICIES.keys())}."
            policies = RANDAUG_POLICIES[policies]
        assert isinstance(policies, list) and len(policies) > 0, "Policies must be a non-empty list."
        assert isinstance(
            num_policies, int
        ), f"Number of policies must be of int type, got {type(num_policies)} instead."
        assert isinstance(
            magnitude_level, (int, float)
        ), f"Magnitude level must be of int or float type, got {type(magnitude_level)} instead."
        assert isinstance(
            total_level, (int, float)
        ), f"Total level must be of int or float type, got {type(total_level)} instead."

        assert isinstance(
            magnitude_std, (Number, str)
        ), f"Magnitude std must be of number or str type, got {type(magnitude_std)} instead."
        if isinstance(magnitude_std, str):
            assert magnitude_std == "inf", f'Magnitude std must be of number or "inf", got "{magnitude_std}" instead.'
    ...
    def _check_policies(self, policies):
        for policy in policies:
            ...
            if magnitude_key is not None:
                ...
                ), f"`magnitude_range` of RandAugment policy {type_name} should be a Sequence with two numbers."
...
class RandAugment_timm(object):
    ...
    def apply_pair(self, img, mask):
        """Apply transforms to a pair of image and mask."""
        # RandAugment_timm only does color augmentations, apply only to image
        img = self.trans(img)
        return img, mask
...
class Shear(object):
    ...
    def __init__(
        ...
        assert isinstance(
            magnitude, (int, float)
        ), f"The magnitude type must be int or float, but got {type(magnitude)} instead."
        if isinstance(pad_val, int):
            pad_val = tuple([pad_val] * 3)
        elif isinstance(pad_val, Sequence):
            assert len(pad_val) == 3, f"pad_val as a tuple must have 3 elements, got {len(pad_val)} instead."
        ...
        assert (
            0 <= random_negative_prob <= 1.0
        ), f"The random_negative_prob should be in range [0,1], got {random_negative_prob} instead."
    ...
    def apply_pair(self, img, mask):
        ...
        # Ensure mask is 3-channel for shear if it is single-channel or has a single channel dimension
        if mask.ndim == 2:
            mask_3ch = np.stack([mask] * 3, axis=2)
        elif mask.ndim == 3 and mask.shape[2] == 1:
            # mask shape (H, W, 1) -> repeat to (H, W, 3)
            mask_3ch = np.repeat(mask, 3, axis=2)
        else:
            assert mask.ndim == 3 and mask.shape[2] == 3, f"Got shape: {mask.shape}, but need 3-channel image."
            mask_3ch = mask
        mask_sheared_tmp = mmcv.imshear(
            mask_3ch, magnitude, direction=self.direction, border_value=0, interpolation="nearest"
        )
        # Convert back to single channel by averaging across channels
        mask_sheared = np.mean(mask_sheared_tmp, axis=2).astype(np.uint8)
        img_sheared = img_sheared.astype(np.uint8 if was_img else img.dtype)
        mask_sheared = mask_sheared.astype(np.uint8 if was_img else img.dtype)
        if was_img:
            img_sheared = Image.fromarray(img_sheared)
            mask_sheared = Image.fromarray(mask_sheared).convert("L")
...
class Translate(object):
    ...
    def __call__(self, img):
        ...
        img_translated = img_translated.astype(np.uint8 if was_img else img.dtype)
        if was_img:
            img_translated = Image.fromarray(img_translated)
    ...
    def apply_pair(self, img, mask):
        ...
        mask_translated = mmcv.imtranslate(
            mask, offset, direction=self.direction, border_value=0, interpolation="nearest"
        )
        img_translated = img_translated.astype(np.uint8 if was_img else img.dtype)
        mask_translated = mask_translated.astype(np.uint8 if was_img else mask.dtype)
        if was_img:
            img_translated = Image.fromarray(img_translated)
            mask_translated = Image.fromarray(mask_translated).convert("L")
...
class Rotate(object):
    ...
    def __init__(
        ...
        assert isinstance(angle, float), f"The angle type must be float, but got {type(angle)} instead."
        if isinstance(center, tuple):
            assert len(center) == 2, f"center as a tuple must have 2 elements, got {len(center)} elements instead."
        else:
            assert center is None, f"The center typemust be tuple or None, got {type(center)} instead."
        assert isinstance(scale, float), f"the scale type must be float, but got {type(scale)} instead."
        if isinstance(pad_val, int):
            pad_val = tuple([pad_val] * 3)
        elif isinstance(pad_val, Sequence):
            assert len(pad_val) == 3, f"pad_val as a tuple must have 3 elements, got {len(pad_val)} instead."
...
class Solarize(object):
    ...
    def __init__(self, thr, prob=0.5):
        assert isinstance(thr, (int, float)), f"The thr type must be int or float, but got {type(thr)} instead."
        assert 0 <= prob <= 1.0, f"The prob should be in range [0,1], got {prob} instead."
...
class SolarizeAdd(object):
    ...
    def __init__(self, magnitude, thr=128, prob=0.5):
        assert isinstance(
            magnitude, (int, float)
        ), f"The thr magnitude must be int or float, but got {type(magnitude)} instead."
        assert isinstance(thr, (int, float)), f"The thr type must be int or float, but got {type(thr)} instead."
        assert 0 <= prob <= 1.0, f"The prob should be in range [0,1], got {prob} instead."
...
def cutout(img, shape, pad_val=0):
    ...
    if isinstance(pad_val, (int, float)):
        ...
        ), "Expected the num of elements in tuple equals the channelsof input image. Found {} vs {}".format(
            len(pad_val), channels
        )
    else:
...
class Cutout(object):
    ...
    def __init__(self, shape, pad_val=128, prob=0.5):
        if isinstance(shape, float):
            ...
            raise TypeError(f"shape must be of type int, float or tuple, got {type(shape)} instead")
        if isinstance(pad_val, int):
            pad_val = tuple([pad_val] * 3)
        elif isinstance(pad_val, Sequence):
            assert len(pad_val) == 3, f"pad_val as a tuple must have 3 elements, got {len(pad_val)} instead."

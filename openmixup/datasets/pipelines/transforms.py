...
class CenterCropForEfficientNet(object):
    ...
    def __init__(self, size, efficientnet_style=False, crop_padding=32, interpolation="bilinear", backend="cv2"):
...
class RandomResizedCropForEfficient(object):
    ...
    def __init__(
        ...
        if (scale[0] > scale[1]) or (ratio[0] > ratio[1]):
            raise ValueError(f"range should be of kind (min, max). But received scale {scale} and rato {ratio}.")
    ...
    def __call__(self, img):
        ...
        img = mmcv.imresize(img, tuple(self.size[::-1]), interpolation=self.interpolation, backend=self.backend)
        return Image.fromarray(img.astype(np.uint8))
...
@PIPELINES.register_module()
class RandomResizedCropPair(_transforms.RandomResizedCrop):
    def apply_pair(self, img, mask):
        """Apply the same random crop to both image and mask.

        Args:
            img (PIL.Image): Image to be cropped.
            mask (PIL.Image): Mask to be cropped (same crop as image).

        Returns:
            tuple: (cropped_img, cropped_mask)
        """
        # Get params once for both
        top, left, h, w = self.get_params(img, scale=self.scale, ratio=self.ratio)

        # Apply same crop to both
        img_cropped = F.resized_crop(img, top, left, h, w, self.size, self.interpolation)
        mask_cropped = F.resized_crop(mask, top, left, h, w, self.size, F.InterpolationMode.NEAREST)

        return img_cropped, mask_cropped


@PIPELINES.register_module()
class ResizePair(_transforms.Resize):
    def apply_pair(self, img, mask):
        """Apply the same random crop to both image and mask.
        ...
        img_resized = F.resize(img, self.size, self.interpolation, self.max_size, self.antialias)
        mask_resized = F.resize(mask, self.size, F.InterpolationMode.NEAREST, self.max_size, False)
...
class Resize_mmcls(object):
    ...
    def _resize_img(self, img):
        ...
        if not ignore_resize:
            img = mmcv.imresize(
                img, size=(width, height), interpolation=self.interpolation, return_scale=False, backend=self.backend
            )
        return img
...
class RandomErasing(object):
    ...
    def _erase(self, img, chan, img_h, img_w, dtype):
        ...
        for _ in range(count):
            for _ in range(10):
                target_area = random.uniform(self.min_area_ratio, self.max_area_ratio) * area / count
                ...
                if w < img_w and h < img_h:
                    ...
                    img[:, top : top + h, left : left + w] = self._get_pixels(
                        (chan, h, w), dtype=dtype, device=self.device
                    )
                    break
...
class BEiTMaskGenerator(object):
    ...
    def __call__(self, img: Tuple[torch.Tensor, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor, np.ndarray]:
    ...
    def __call__(self, img=None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.mask_color == "mean":
            if isinstance(img, Image.Image):
                ...
                mask_ = mask_.repeat(self.model_patch_size, axis=0).repeat(self.model_patch_size, axis=1)
...
class RandomResizedCropWithTwoCrop(object):
    ...
    interpolation_dict = {"bicubic": Image.BICUBIC, "lanczos": Image.LANCZOS, "hamming": Image.HAMMING}
    ...
    def __init__(
        ...
        if interpolation == "random":
            ...
            self.interpolation = self.interpolation_dict.get(interpolation, Image.BILINEAR)
        self.second_interpolation = self.interpolation_dict.get(second_interpolation, Image.BILINEAR)
        self.scale = scale
        self.ratio = ratio
    ...
    def __call__(self, img):
        ...
        if self.second_size is None:
            ...
            return F.resized_crop(img, i, j, h, w, self.size, interpolation), F.resized_crop(
...
class Lighting_mmcls(object):
    ...
    def __init__(
        ...
        assert isinstance(alphastd, float), f"alphastd should be of type float or int, got {type(alphastd)} instead."
    ...
    def __call__(self, img):
        if self.alphastd <= 0.0:
            return img
        img = np.array(img)
        img = mmcv.adjust_lighting(img, self.eigval, self.eigvec, alphastd=self.alphastd, to_rgb=False)
        return Image.fromarray(img.astype(np.uint8))
...
class Lighting(object):
    ...
    def __init__(self, eigval=None, eigvec=None, alphastd=0.1):
...
class GaussianBlur(object):
    ...
    def __init__(self, sigma_min=0.1, sigma_max=2.0, p=0.5):
        assert 0 <= p <= 1.0, f"The prob should be in range [0,1], got {p} instead."

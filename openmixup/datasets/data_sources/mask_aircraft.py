...
class MaskAircraft(object):
    """FGVC-Aircraft dataset with foreground masks from a ZIP file.
    ...
    def __init__(
        ...
        if self.mask_zip is None:
            self.logger.warning(
                "MaskAircraft: mask_zip is None, all masks will be white "
                "(foreground=all)"
            )
        ...
        self.logger.info(
            f"Initialize MaskAircraft: root={root}, masks={self.mask_zip}, "
            f"split={split}"
        )
    ...
    def get_sample(self, idx):
        img_path = self.dataset._image_files[idx]
        label = self.dataset._labels[idx]

...
from .pipelines.compose import BuildCompose
...
class ClassificationDataset(BaseDataset):
    ...
    def __init__(self, data_source, pipeline, prefetch=False):
        ...
        self.pipeline = BuildCompose(pipeline)
    ...
    def __getitem__(self, idx):
        sample = self.data_source.get_sample(idx)
        ...
        assert isinstance(img, Image.Image), (
            "The output from the data source must be an Image, got: {}. Please ensure that the list file"
            " does not contain labels.".format(type(img))
        )

        if mask is not None:
            # Use apply_pair to ensure same random transforms for image and mask
            img, mask = self.pipeline.apply_pair(img, mask)
            if self.prefetch:
                img = torch.from_numpy(to_numpy(img))
                mask = torch.from_numpy(to_numpy(mask))
            return dict(img=img, mask=mask, gt_label=target, idx=idx)
        else:
    ...
    def evaluate(self, scores, keyword, logger=None, metric="accuracy", metric_options=None, topk=(1, 5), **kwargs):
        ...
        if len(set(metrics) & set(precision_recall_f1_keys)) != 0:
            ...
            if thrs is not None:
                precision_recall_f1_values = precision_recall_f1(scores, target, average_mode=average_mode, thrs=thrs)
            else:
                precision_recall_f1_values = precision_recall_f1(scores, target, average_mode=average_mode)
            for key, values in zip(precision_recall_f1_keys, precision_recall_f1_values):
                if key in metrics:
                    if isinstance(thrs, tuple):
                        eval_res.update({f"{key}_thr_{thr:.2f}": value for thr, value in zip(thrs, values)})
                    else:
                        eval_res[key] = values
    ...
    def ece_score(self, py, n_bins=10, save_name=None, show_plot=True):

...
class ValidateHook(Hook):
    ...
    def __init__(
        ...
        **eval_kwargs,
        ...
        if isinstance(dataset, Dataset):
            ...
            raise TypeError("dataset requires a Dataset or a dict, not {}".format(type(dataset)))
        ...
        assert (
            isinstance(save_best, str) or save_best is None
        ), f'""save_best"" should be a str or None rather than {type(save_best)}'
    ...
    def _save_ckpt(self, runner, key_score):
        ...
        if self.compare_func(key_score, best_score):
            ...
            runner.save_checkpoint(self.out_dir, filename_tmpl=best_ckpt_name, create_symlink=False)
    ...
    def _run_validate(self, runner):
        ...
        for name, val in results.items():
            ...
            if self.save_val:
                np.save(f"{runner.work_dir}/val_{name}_epoch_{runner.epoch+1}.npy", val)
    ...
    def _evaluate(self, runner, results, keyword):
        ...
        eval_res = self.dataset.evaluate(
            results, keyword=keyword, logger=runner.logger, **self.eval_kwargs["eval_param"]
        )
        ...
        if self.save_best is not None:
            ...
            try:
                ...
                first_key = list(eval_res.keys())[0]
                runner.logger.warning(
                    f"KeyError ({e}) for key {self.key_indicator} on dict {eval_res} use key {first_key} instead"
                )
                return eval_res[first_key]

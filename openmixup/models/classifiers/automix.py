    ...
    *** Requiring Hook: `momentum_update` is adjusted by `CosineScheduleHook` after
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.mask_up_override = mask_up_override if isinstance(mask_up_override, (str, list)) else None
        self.save = bool(save)
        ...
        if backbone_k is not None:
            ...
            assert self.momentum >= 1.0 and pretrained_k is not None
        else:
        ...
        self.weight_mix_q = head_weights.get("head_mix_q", 1.0)
        self.weight_mix_k = head_weights.get("head_mix_k", 1.0)
        self.weight_one_q = head_weights.get("head_one_q", 1.0)
    ...
    def _update_loss_weights(self):
        ...
        if len(self.head_weights["decent_weight"]) > 0:
            for attr in self.head_weights["decent_weight"]:
                setattr(self, attr, self.head_weights.get(attr, 1.0) * self.cos_annealing)
        # cos annealing accent, from 0 to 1
        if len(self.head_weights["accent_weight"]) > 0:
            for attr in self.head_weights["accent_weight"]:
                setattr(self, attr, self.head_weights.get(attr, 1.0) * (1 - self.cos_annealing))
    ...
    def _no_repeat_shuffle_idx(self, batch_size_this, ignore_failure=False):
        ...
        if idx_repeat == True and ignore_failure == False:
            ...
            idx_shuffle = torch.tensor(  # shift the original idx
                [(i + idx_shift) % batch_size_this for i in range(batch_size_this)]
            ).cuda()
        return idx_shuffle
    ...
    def forward_train(self, img, gt_label, **kwargs):
        ...
        if self.save and self.mask_adjust > 0:
            self.plot_mix(results["img_mix_bb"], img, img[index_bb, :], lam[1], results["debug_plot"], "backbone")
        # save img mb
        if self.save and self.mask_adjust <= 0:
            self.plot_mix(results["img_mix_mb"], img, img[index_mb, :], lam[0], results["debug_plot"], "mixblock")
    ...
    def plot_mix(self, im_mixed, im_q, im_k, lam, debug_plot=None, name="k"):
        ...
        self.ploter.plot(
            img, nrow=4, title_name=title_name, save_name=self.save_name, make_grid=not self.save_by_sample
        )
    ...
    def forward_k(self, mixed_x, y, index, lam):
        ...
        if self.pre_mix_loss > 0:
            ...
            loss_mix_k["pre_mix_loss"] = self.mix_block.pre_head.loss(pred_mix_mb, y_mix_k)["loss"] * self.pre_mix_loss
    ...
    def pixel_mixup(self, x, y, lam, index, feature):
        ...
        # more robust scale factor:
        scale_factor = x.shape[2] // feature.shape[2]
        assert (
            x.shape[2] % feature.shape[2] == 0
        ), f"Input spatial size {x.shape[2]} must be divisible by feature size {feature.shape[2]}"
        ...
        mask_mb = self.mix_block(
            ...
            debug=self.debug,
            unsampling_override=self.mask_up_override,
        )
        mask_bb = self.mix_block(
            feature, lam_bb, index[1], scale_factor=scale_factor, debug=False, unsampling_override=None
        )
        if self.debug:
        ...
        if self.pre_one_loss > 0.0:
            pred_one = self.mix_block.pre_head([mask_mb["x_lam"]])
            y_one = (y, y, 1)
            results["pre_one_loss"] = self.mix_block.pre_head.loss(pred_one, y_one)["loss"] * self.pre_one_loss
        ...
        if self.mask_adjust > np.random.rand():  # [0,1)
            ...
            # if mean > lam_bb
            mask_bb[idx_larg == True, 0, :, :] = _mask[idx_larg == True, :, :] * (
                lam[0] / _mean[idx_larg == True].view(-1, 1, 1)
            )
            mask_bb[idx_larg == True, 1, :, :] = 1 - mask_bb[idx_larg == True, 0, :, :]
            # elif mean < lam_bb
            mask_bb[idx_less == True, 1, :, :] = (1 - _mask[idx_less == True, :, :]) * (
                (1 - lam[0]) / (1 - _mean[idx_less == True].view(-1, 1, 1))
            )
        ...
        assert mask_mb.shape[2:] == x.shape[2:], (
            f"Invalid mask shape={mask_mb.shape} for input shape={x.shape} with scale factor={scale_factor} on feature"
    ...
    def simple_test(self, img, **kwargs):
        ...
        if self.head_ensemble:
            pred.append([torch.stack([pred[i][0] ** 2 for i in range(len(pred))]).mean(dim=0)])
            keys.append("acc_avg")

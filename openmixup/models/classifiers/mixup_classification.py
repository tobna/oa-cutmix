...
class MixUpClassification(BaseModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.cos_annealing = 1.0  # decent from 1 to 0 as cosine
        ...
        self.dynamic_mode = {
            ...
            "attentivemix_fga": attentivemix_fga,
            "puzzlemix": puzzlemix,
            "automix": self._mixblock,
            "samix": self._mixblock,
            "snapmix": snapmix,
            "guidedmix": guidedmix,
        ...
        self.mix_args = dict(  # default settings
            ...
            attentivemix_fga=dict(grid_size=32, top_k=6, beta=8, fg_weight=1, mode="absolute"),
            mask_attentivemix=dict(grid_size=32, top_k=6, fg_weight=1.0, mode="absolute"),
        ...
        if self.mix_prob is not None:
            assert (
                len(self.mix_prob) == len(self.alpha) and abs(sum(self.mix_prob) - 1e-10) <= 1
        ...
        if len(self.mix_mode) < self.mix_repeat:
            print_log(
                "Warning: the number of mix_mode={} is less than mix_repeat={}.".format(self.mix_mode, self.mix_repeat)
            )
    ...
    def _momentum_update(self):
        ...
        for param_q, param_k in zip(self.backbone.parameters(), self.backbone_k.parameters()):
            if self.momentum_k >= 1:
                ...
                param_k.data = param_k.data * self.momentum_k + param_q.data * (1.0 - self.momentum_k)
    ...
    def _features(self, img, gt_label=None, cur_mode="puzzlemix", **kwargs):
        ...
        if cur_mode in ["attentivemix", "attentivemix_fga"]:
            img = F.interpolate(img, scale_factor=kwargs.get("feat_size", 224) / img.size(2), mode="bilinear")
            ...
            if cur_mode == "puzzlemix":
                ...
                # Requires ClsMixupHead (always exposes .fc)
                weight = self.head.fc.weight.data  # (num_classes, C)
                last_feat = self.backbone(img)[-1]
                # ViT: last_feat is [patch_token, cls_token[, attn]]
                # patch_token is already (B, C, H_p, W_p) after backbone permute
                if isinstance(last_feat, (list, tuple)):
                    last_feat = last_feat[0]
                # Compute GT-class CAM: (B, H, W)
                if gt_label.dim() == 1:
                    class_weights = weight[gt_label]  # (B, C)
                else:
                    class_weights = gt_label.float() @ weight  # (B, C)
                cam = F.relu(torch.einsum(
                    'nc,nchw->nhw', class_weights, last_feat))
                cam = F.interpolate(
                    cam.unsqueeze(1), size=img.shape[2:],
                    mode='bilinear', align_corners=False).squeeze(1)
                features = (
                    cam / (cam.sum(dim=(1, 2), keepdim=True) + 1e-8)
                ).detach()
    ...
    def _manifoldmix(self, img, gt_label, alpha, cur_mode="manifoldmix"):
        ...
        _layer = np.random.randint(self.mix_args[cur_mode]["layer"][0], self.mix_args[cur_mode]["layer"][1], dtype=int)
        # generate mixup mask
        _mask = None
    ...
    def forward_mix(self, img, gt_label, remove_idx=-1, mask=None):
        ...
        Args:
            ...
            mask (Tensor, optional): Object masks of shape (N, 1, H, W).
                Only used when mix_mode includes 'mask_mixup'.
        ...
        return_mask = False  # return sample mixup mask in [N, 1, H, W]
        ...
        if cur_mode in ["attentivemix", "attentivemix_fga", "automix", "puzzlemix", "samix", "snapmix", "guidedmix"]:
            if cur_mode in ["attentivemix", "attentivemix_fga", "puzzlemix", "snapmix", "guidedmix"]:
                features = self._features(img, gt_label=gt_label, cur_mode=cur_mode, **self.mix_args[cur_mode])
                mix_args = dict(
                    alpha=cur_alpha,
                    dist_mode=False,
                ...
                if cur_mode == "attentivemix_fga":
                    mix_args["obj_mask"] = mask
            ...
            if isinstance(x, (list, tuple)):  # e.g. output_cls_token=True
                x = x[0]  # patch_token [B, C, H, W]
            ...
            if cur_mode == "mixpro":
                ...
                if self.offical_set is True and return_mask is False:
                    raise ValueError(
                        "Please make sure the return mask if 'True' if you want use offical loss computating !         "
                        "                             Now, the setting is: return mask:{} and offical set: {}".format(
                            return_mask, self.offical_set
                        )
                    )
                if return_mask:
                    x_ = img.clone().detach()
        ...
        # applying hand-crafted sample mixup methods
            ...
            if cur_mode in [
                ...
                if cur_mode in ["mask_mixup", "cutmix_foreground_area"]:
                    mix_args = dict(alpha=cur_alpha, dist_mode=False, mask=mask, **self.mix_args[cur_mode])
                    if mask is None:
                        print("Error, got mask None in forward_mix before calling mixup!", flush=True)
    ...
    def forward_train(self, img, gt_label, **kwargs):
        ...
        Args:
            ...
            kwargs: Any keyword arguments to be used to forward.
                May contain 'mask' for mask_mixup.
        ...
        for i in range(self.mix_repeat):
            ...
            if i == 0:
                losses, cur_idx = self.forward_mix(img.clone(), gt_label, remove_idx=remove_idx, mask=mask)
            else:
                _loss, cur_idx = self.forward_mix(img.clone(), gt_label, remove_idx=remove_idx, mask=mask)
    ...
    def forward_vit_based_mix(self, pred, gt_label, extra_var=None, mode=None):
        ...
        if mode == "mixpro":  # mixpro needs smooth_labels compute mixing ratio lambda
            ...
            lam = (sim) * lam_ + (1.0 - sim) * gt_label[-1]
            ...
            losses["loss"] += (
                self.head.loss(pred_target, gt_label[0])["loss"] + self.head.loss(pred_scoure, gt_label[1])["loss"]
            )
    ...
    def plot_mix(self, img_mixed, mix_mode="", lam=None):
        ...
        title_name = "{}, lam={}".format(mix_mode, lam)  # if isinstance(lam, float) else mix_mode

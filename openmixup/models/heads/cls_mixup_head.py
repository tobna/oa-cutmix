...
class ClsMixupHead(BaseModule):
    ...
    def __init__(
        ...
        self.extract_cls_token = bool(extract_cls_token)
        assert not (
            self.with_avg_pool and self.extract_cls_token
        ), f"cant do average pooling AND class token extraction"
        ...
        assert (
            eta_weight["mode"] in ["more", "less", "both"] and 0 <= eta_weight["thr"] <= 1 and eta_weight["eta"] < 100
        )
        assert 0 < lam_thr <= 1 and -100 < lam_idx < 100
        assert 0 < two_hot_scale <= 1 and 0 <= neg_weight <= 1

        print(
            "DEBUG init cls_mixup_head with"
            + str(
                dict(
                    ...
                    two_hot=two_hot,
                    two_hot_scale=two_hot_scale,
                    lam_scale_mode=lam_scale_mode,
                    lam_thr=lam_thr,
                    lam_idx=lam_idx,
        ...
        if self.neg_weight != 1:
            0 <= self.neg_weight <= 1, "the weight of negative parts should not be                 larger than the postive part."
    ...
    def init_weights(self, init_linear="normal", std=0.01, bias=0.0):
        if self.init_cfg is not None:
            ...
            return
        assert init_linear in ["normal", "kaiming", "trunc_normal"], "Undefined init_linear: {}".format(init_linear)
    ...
    def forward_head(self, x, post_process=False):
        ...
        if self.with_avg_pool:
            if x.dim() == 3:
                ...
                assert x.dim() in [2, 3, 4], "Tensor must has 2, 3 or 4 dims, got: {}".format(x.dim())
        x = self.fc(x)
        if post_process:
    ...
    def forward(self, x, post_process=None, **kwargs):
        ...
        if self.extract_cls_token:
            ...
            return [self.forward_head(x, post_process)]

        if isinstance(x, list):
            if isinstance(x[-1], (tuple, list)) and len(x[-1]) == 2:  # For SwinTransformer, without attention-maps
                x, _ = x[-1]
                x = [x]
    ...
    def lambda_adjust(self, lam, mode="pow", thr=1, idx=1):
        ...
        if mode == "pow":
            lam = torch.where(lam >= thr, 1.0, (thr ** (-abs(idx))) * (lam ** abs(idx)))
        elif mode == "exp":
            b = (abs(idx) ** (-thr * 2)) * 1
            k = 1 / (1 - b)
            lam = torch.where(lam >= thr, 1.0, ((abs(idx) ** (lam - thr * 2)) * (abs(idx) ** lam) - b) * k)
        else:
    ...
    def co_loss(self, cls_score, labels, **kwargs):
        ...
        single_label = y[0].dim() == 1 or (y[0].dim() == 2 and y[0].shape[1] == 1)
        ...
        if not self.multi_label:
            ...
            losses["loss"] = self.criterion(
                ...
                class_weight_override=class_weight,
                eta_weight=use_eta_weight,
                **kwargs,
            )
        # compute accuracy
    ...
    def loss(self, cls_score, labels, label_mask=None, multi_lam=False, **kwargs):
        ...
        if not isinstance(labels, tuple):
            ...
            single_label = labels.dim() == 1 or (labels.dim() == 2 and labels.shape[1] == 1)
        ...
        # 2. mixup classification
            ...
            if isinstance(lam, torch.Tensor):  # lam is scalar or tensor [N,\*]
                lam = lam.view(-1, 1)
            # whether is the single label cls [N,] or multi-label cls [N,C]
            single_label = y_a.dim() == 1 or (y_a.dim() == 2 and y_a.shape[1] == 1)
            # Notice: we allow the single-label cls using multi-label loss, thus
            # * For single-label or multi-label cls, loss = loss.sum() / N
            avg_factor = y_a.size(0)

            # 2.1 mixup (hard ce) cls (using softmax)
            _lam_is_vec = isinstance(lam, torch.Tensor) and lam.numel() > 1
            if not self.multi_label and len(labels) == 3:
                if _lam_is_vec:
                    loss_a = self.criterion(cls_score, y_a, reduction_override="none", **kwargs)
                    loss_b = self.criterion(cls_score, y_b, reduction_override="none", **kwargs)
                    losses["loss"] = torch.mean(loss_a * lam.view(-1) + loss_b * (1 - lam).view(-1))
                else:
                    losses["loss"] = self.criterion(cls_score, y_a, avg_factor=avg_factor, **kwargs) * lam + self.criterion(
                        cls_score, y_b, avg_factor=avg_factor, **kwargs
                    ) * (1 - lam)
            elif len(labels) == 4:  # two independent lambdas (e.g. snapmix)
                if _lam_is_vec:
                    loss_a = self.criterion(cls_score, y_a, reduction_override="none", **kwargs)
                    loss_b = self.criterion(cls_score, y_b, reduction_override="none", **kwargs)
                    losses["loss"] = torch.mean(loss_a * lam.view(-1) + loss_b * lam_.view(-1))
                else:
                    losses["loss"] = (
                ...
                if self.two_hot:
                    if self.lam_scale_mode != "none":
                        lam_a = self.lambda_adjust(lam, mode=self.lam_scale_mode, thr=self.lam_thr, idx=self.lam_idx)
                ...
                # 2.3 mixup (soft) single-label sumed to 1 (using softmax)
                    ...
                    if self.eta_weight["eta"] != 0:
                        if not isinstance(lam, torch.Tensor):
                            ...
                            lam = lam.view(y_a.size(0))
                        # whether to use eta
                        below_thr = lam < self.eta_weight["thr"]
                        if self.eta_weight["mode"] == "less":
                            use_eta_weight = [torch.where(below_thr, lam, 0.0), torch.where(below_thr, 0.0, 1 - lam)]
                        elif self.eta_weight["mode"] == "more":
                            use_eta_weight = [torch.where(below_thr, 0.0, lam), torch.where(below_thr, 1 - lam, 0.0)]
                        else:
                            use_eta_weight = [lam, 1 - lam]  # 'both'
                        # eta rescale by lam
                        for i in range(len(use_eta_weight)):
                            if self.lam_scale_mode != "none":
                                use_eta_weight[i] = torch.where(
                                    use_eta_weight[i] > 0,
                                    self.eta_weight["eta"]
                                    * self.lambda_adjust(
                                        use_eta_weight[i], mode=self.lam_scale_mode, thr=self.lam_thr, idx=self.lam_idx
                                    ),
                                    0.0,
                                )
                            else:
                                use_eta_weight[i] = torch.where(use_eta_weight[i] > 0, self.eta_weight["eta"], 0.0)
                            use_eta_weight[i] = use_eta_weight[i].to(y_mixed.device)
                        is_allowed = (use_eta_weight[0] > 0) | (use_eta_weight[1] > 0)
                        if self.eta_weight["mode"] == "less":
                            is_allowed = is_allowed | (lam == 0.0)
                        assert is_allowed.all(), f"one of eta should be non-zero, lam={lam}, lam_={1 - lam}"
...
class ClsUncertainMixupHead(BaseModule):
    ...
    def IN_loss(self, score, labels, rand_index, **kwargs):
        ...
        losses["loss"] = torch.mean(
            self.criterion(cls_mix, y_a, avg_factor=avg_factor) * lam_a
            + self.criterion(cls_mix, y_b, avg_factor=avg_factor) * lam_b
        ) + self.gama * (
            self.criterion(INa_f, y_a, avg_factor=avg_factor) * lam
                    ...
                    if self.eta_weight["eta"] != 0:
                        ...
                        for i in range(len(use_eta_weight)):
                            if use_eta_weight[i] > 0:
                                if self.lam_scale_mode != "none":
                                    use_eta_weight[i] = self.eta_weight["eta"] * self.lambda_adjust(
                                        use_eta_weight[i], mode=self.lam_scale_mode, thr=self.lam_thr, idx=self.lam_idx
                                    )
                                else:
                                    use_eta_weight[i] = self.eta_weight["eta"]
                                assert (
                                    use_eta_weight[0] > 0 or use_eta_weight[1] > 0

...
def mixpro(
    ...
    **kwargs,
    ...
    def mixup_target(y_a, y_b, num_classes, lam, smoothing=0.1, device="cuda"):
        ...
        assert y1.shape == y2.shape, f"y1 and y2 must have same shape, got {y1.shape} and {y2.shape}"
        assert y1.shape[1] == num_classes, f"y1 must have num_classes={num_classes} in dim 1, got {y1.shape[1]}"
        assert (
            lam.shape[0] == y1.shape[0]
        ), f"lam batch size must match y1 batch size, got {lam.shape[0]} and {y1.shape[0]}"
    ...
    if not dist_mode:
        ...
        # print(
        #     f"[DEBUG mixpro] num_classes: {num_classes}, y_a shape: {y_a.shape}, y_b shape: {y_b.shape}, y_a unique:"
        #     f" {y_a.unique()}, lam: {lam.shape}"
        # )

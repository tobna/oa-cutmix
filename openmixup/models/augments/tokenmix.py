...
def tokenmix(
    ...
    **kwargs,
    ...
    def generate_mask(lam, mask_token_num_start, min_num_patches=1):
        width, height = token_w_h, token_w_h
        min_aspect = 0.3
    ...
    def generate_mask_random(lam, mask_token_num_start=token_w_h):
        width, height = token_w_h, token_w_h
        mask = np.zeros(shape=(width * height), dtype=np.int32)
        mask_ratio = 1 - lam
        ...
        mask_idx = np.random.permutation(token_w_h * token_w_h)[:num_masking_patches]
        mask[mask_idx] = 1
    ...
    if not dist_mode:
        ...
        if attn is not None:
            try:
                attn = torch.mean(attn[:, :, 0, 1:], dim=1).reshape(b, 1, token_w_h, token_w_h)
            except IndexError as e:
                print(f"error indexing attention matrix of shape {attn.shape}: {e}")
                raise e

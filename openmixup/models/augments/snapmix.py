...
@torch.no_grad()
def snapmix(img, gt_label, alpha=1.0, lam=None, dist_mode=False, features=None, **kwargs):
    ...
    b, _, h, w = img.size()
    lam_a = torch.ones(img.size(0), device=img.device)
    lam_b = 1 - lam_a
    ...
    if not dist_mode:
        ...
        lam_a = lam_a.to(y_a.device)
        lam_b = lam_b.to(y_b.device)

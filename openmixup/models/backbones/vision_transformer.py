...
class VisionTransformer(BaseBackbone):
    ...
    def init_weights(self, pretrained=None):
        if isinstance(pretrained, str):
            ...
            # Load raw checkpoint to handle non-standard wrapper keys
            # (e.g. DeiT III / timm exports wrap weights under 'model')
            if pretrained.startswith('http://') or \
                    pretrained.startswith('https://'):
                import torch.hub
                checkpoint = torch.hub.load_state_dict_from_url(
                    pretrained, map_location='cpu', check_hash=False)
            else:
                checkpoint = torch.load(pretrained, map_location='cpu')
            if 'model' in checkpoint and \
                    not any(k.startswith('backbone') for k in checkpoint):
                state_dict = checkpoint['model']
            elif 'state_dict' in checkpoint:
                state_dict = checkpoint['state_dict']
            else:
                state_dict = checkpoint
            # DeiT III / timm checkpoints store pos_embed without the
            # cls-token slot: shape [1, num_patches, C] instead of
            # [1, num_patches + num_extra_tokens, C].  Prepend zeros so
            # that _prepare_pos_embed can handle it correctly.
            # Remap timm-style keys (DeiT III / vanilla timm exports) to
            # mmcv VisionTransformer naming conventions.
            if any(k.startswith('blocks.') for k in state_dict):
                ...
                for k, v in state_dict.items():
                    ...
                    k = re.sub(r'^blocks\.(\d+)\.norm1\.', r'layers.\1.ln1.', k)
                    k = re.sub(r'^blocks\.(\d+)\.norm2\.', r'layers.\1.ln2.', k)
                    k = re.sub(r'^blocks\.(\d+)\.attn\.', r'layers.\1.attn.', k)
                    k = re.sub(r'^blocks\.(\d+)\.mlp\.fc1\.', r'layers.\1.ffn.layers.0.0.', k)
                    k = re.sub(r'^blocks\.(\d+)\.mlp\.fc2\.', r'layers.\1.ffn.layers.1.', k)
                    k = re.sub(r'^blocks\.(\d+)\.(gamma_\d)$', r'layers.\1.\2', k)
                    if k in ('norm.weight', 'norm.bias'):
                        k = k.replace('norm.', 'ln1.')
                    remapped[k] = v
                state_dict = remapped
                print_log('Remapped timm-style keys to mmcv VisionTransformer keys',
                          logger=logger)
            # Strip backbone. prefix from mmcls-style checkpoints so that
            # VisionTransformer can load them directly when called standalone.
            ...
            if 'pos_embed' in state_dict:
                ...
                expected = self.num_patches + self.num_extra_tokens
                if pe.shape[1] == self.num_patches and self.num_extra_tokens > 0:
                    prefix = pe.new_zeros(1, self.num_extra_tokens, pe.shape[2])
                    state_dict['pos_embed'] = torch.cat([prefix, pe], dim=1)
                    print_log(
                        f'Prepended {self.num_extra_tokens} zero extra-token '
                        f'slot(s) to pos_embed: {pe.shape} -> '
                        f'{state_dict["pos_embed"].shape}',
                        logger=logger)
            incompatible = self.load_state_dict(state_dict, strict=False)
            if incompatible.missing_keys:
                print_log(
                    f'Missing keys in backbone checkpoint: '
                    f'{incompatible.missing_keys}',
                    logger=logger, level=logging.WARNING)
            if incompatible.unexpected_keys:
                print_log(
                    f'Unexpected keys in backbone checkpoint: '
                    f'{incompatible.unexpected_keys}',
                    logger=logger, level=logging.WARNING)
            if not incompatible.missing_keys and not incompatible.unexpected_keys:
                print_log('Backbone checkpoint loaded with no key mismatches.',
                          logger=logger)
        else:

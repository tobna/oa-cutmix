...
def cross_entropy(pred, label, weight=None, reduction="mean", avg_factor=None, class_weight=None, **kwargs):
...
def soft_cross_entropy(pred, label, weight=None, reduction="mean", class_weight=None, avg_factor=None, **kwargs):
...
def soft_mix_cross_entropy(
    ...
    **kwargs,
    ...
    mix_num = max(max(mask_one), 2)  # 1 label means not mixed
    mask_one = mask_one >= mix_num
    if mask_one.sum() < label.size(0):
        pred_one = pred[~mask_one, :]
        label_one = label[~mask_one, :]
        pred = pred[mask_one, :]
        label = label[mask_one, :]
        if eta_weight is not None and isinstance(eta_weight[0], torch.Tensor) and eta_weight[0].numel() > 1:
            eta_weight = [w[mask_one] for w in eta_weight]
        weight_one = None
        if weight is not None:
            weight_one = weight[~mask_one, ...].float()
    ...
    if pred.size(0) == 0:
        ...
        logger.warning(f"Got no mixed samples in batch. Label weights: {label.unique()}")
        loss = 0.0
    else:
        ...
        # lam_k = torch.zeros((label.size(0), mix_num), dtype=label.dtype, device=label.device)
        # for i in range(label.size(0)):
        #     lam_k[i] = label[i, label[i] > _eps]  # [k,] k-mix relevant classes
        lam_k_indices = (label > _eps).nonzero()[:, 1].reshape(label.size(0), mix_num)
        ...
        assert len(eta_weight) == lam_k_indices.size(-1), (
            f"eta_weight: {[w.shape for w in eta_weight]}, lam_k_indices: {lam_k_indices.shape}, bs: {bs}, cls_num:"
            f" {cls_num}, mix_num: {mix_num}, mask_one: {(mask_one * 1.).mean()}"
        )
        for i in range(lam_k_indices.size(-1)):
            # selected (C-k+1), except lam_k[j], where j!=i (k-1)
            label_mask = torch.zeros(bs, cls_num, dtype=torch.bool, device=mask_lam_k.device)
            label_mask[torch.arange(bs), lam_k_indices[:, i]] = True

            mask_lam_i = label_mask | ~mask_lam_k  # [N, N]
            pred_lam_i = pred[mask_lam_i].reshape([bs, cls_num + 1 - lam_k_indices.size(-1)])  # [N, C-k+1]
            label_lam_i = label[mask_lam_i].reshape([bs, cls_num + 1 - lam_k_indices.size(-1)])  # [N, C-k+1]
            # convert to onehot
            label_lam_i = (label_lam_i > 0).type(torch.float)
            # element-wise losses
            loss_global = -label_lam_i * F.log_softmax(pred_lam_i, dim=-1)  # [N, C-k+1]
            if class_weight is not None:
                loss_global *= class_weight
            # eta reweight
            # print(
            #     f"lam_k: {lam_k_indices.shape}, mask_lam_i: {mask_lam_i.shape}, pred_lam_i: {pred_lam_i.shape},"
            #     f" loss_global: {loss_global.shape} after sum: {loss_global.sum(dim=-1).shape}, loss: {loss.shape},"
            #     f" eta_weight: {eta_weight[i].shape}",
            #     flush=True,
            # )
    ...
    if label_one is not None:
        ...
        if not isinstance(loss, torch.Tensor):
            ...
            loss = torch.cat([loss, loss_one], dim=0)

    if weight is not None and weight_one is not None:
        weight = torch.cat([weight, weight_one], dim=0)
...
def binary_cross_entropy(pred, label, weight=None, reduction="mean", avg_factor=None, class_weight=None, **kwargs):
...
class CrossEntropyLoss(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.use_mix_decouple:
            assert use_soft, "use_mix_decouple requires 'use_soft' to be true"
        ...
        if self.use_sigmoid:
            ...
            self.criterion = soft_mix_cross_entropy if self.use_mix_decouple else soft_cross_entropy
        else:
    ...
    def forward(
        ...
        **kwargs,
        ...
        class_weight = class_weight_override if class_weight_override is not None else self.class_weight
        if class_weight is not None:
        ...
        if self.use_mix_decouple:
            assert eta_weight is not None, "use_mix_decouple requires 'eta_weight' to be not None"

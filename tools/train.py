...
def parse_args():
    ...
    parser.add_argument("--pretrained", default=None, help="pretrained model file for backbone")
    parser.add_argument("--load_checkpoint", default=None, help="load the checkpoint file of full models")
...
def main():
    ...
    if args.work_dir is not None:
        ...
        omx_path = osp.abspath(osp.join(__file__, "..", ".."))
        config_path = osp.abspath(args.config)
        assert config_path.startswith(
            omx_path
        ), f"config path ({config_path}) has to be below openmixup base path ({omx_path})."
        rel_path = config_path[len(omx_path) + 1 :].split(osp.sep)[1:]  # remove base path and configs base folder
        rel_path[-1] = osp.splitext(rel_path[-1])[0]
        cfg.work_dir = osp.join("./work_dirs", *rel_path, timestamp)
    ...
    if args.launcher == "none":
        ...
        if args.launcher == "slurm":
            ...
            cfg.dist_params["port"] = os.environ.get("MASTER_PORT", args.port)
            # cfg.dist_params["addr"] = os.environ.get("MASTER_ADDR")
    ...
    if cfg.checkpoint_config is not None:
        ...
        cfg.checkpoint_config.meta = dict(openmixup_version=__version__, config=cfg.pretty_text)

    train_model(model, datasets, cfg, distributed=distributed, timestamp=timestamp, meta=meta)
